...
import com.pakpobox.zxinglib.R;
...
public class AutoScannerView extends View {
    ...
    private final int triAngleColor = Color.parseColor("#29D1FF");                        //边角的颜色
    private final int lineColor = Color.parseColor("#29D1FF");                            //中间线的颜色
    private final int textColor = Color.parseColor("#FFFFFF");                            //文字的颜色
    private final int triAngleLength = dp2px(30);                                         //每个角的点距离
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String text = "Scan the QR code on the machine";
...
//            canvas.drawLine(frame.left, frame.top + lineOffsetCount, frame.right, frame.top + lineOffsetCount, linePaint);    //画一条红色的线
            ...
            canvas.drawBitmap(((BitmapDrawable)(getResources().getDrawable(R.drawable.scan_line))).getBitmap(), null, lineRect, linePaint);
        }
