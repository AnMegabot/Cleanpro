...
import com.bigkoo.pickerview.adapter.MonthNumericWheelAdapter;
...
public class WheelTime {
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (startYear == endYear) {//开始年等于终止年
            wv_month.setAdapter(new MonthNumericWheelAdapter(startMonth, endMonth));
            wv_month.setCurrentItem(month + 1 - startMonth);
        } else if (year == startYear) {
            //起始日期的月份控制
            wv_month.setAdapter(new MonthNumericWheelAdapter(startMonth, 12));
            wv_month.setCurrentItem(month + 1 - startMonth);
        } else if (year == endYear) {
            //终止日期的月份控制
            wv_month.setAdapter(new MonthNumericWheelAdapter(1, endMonth));
            wv_month.setCurrentItem(month);
        } else {
            wv_month.setAdapter(new MonthNumericWheelAdapter(1, 12));
            wv_month.setCurrentItem(month);
        }
        wv_month.setGravity(gravity);
        // 日
